...
> **运行时状态**：❌ 不被加载 / ❌ 不立即落地（与 fili-web 体系存在结构性差异，需作为专项任务规划）
> **保留原因**：fili-web 当前体系的**重要补充蓝图**，含多个关键缺口的解决方案
...
### 3.1 题材锚点系统（fili-web 最大缺口）

fili-web 24 个方法模块**主要是叙事结构**（Save the Cat / Truby 22 / 雪花法），**不是题材风格规范**。天命的 19 题材锚点提供了"**写什么**"的答案，与现有"**怎么写**"形成双轴。
...
每个题材独立设计：连续 3 句"我"开头需切换 / 多人场景必须用姓名 / 等。这是 fili-web 当前完全没有的文法层约束。
...
这种细粒度长度规范在 fili-web 当前完全没有。

---

## 四、关键观察 · 思想印证

天命的多处设计**印证了 fili-web 现有方向的正确性**：
...
| 内容 | 不直接采纳原因 |
|---|---|
| 具体 prompt 文本 | fili-web 流水线节点结构与天命不同，prompt 需重写而非套用 |
| 字段命名（如 `coreEvents` / `volumeEndHook`） | 应与 fili-web 现有 `pipeline/types.ts` 命名规范统一 |
| 19 个题材 prompt 的开头（"你是一位专业的 XX 写作助手"） | 与 fili-web 角色化 system prompt 设计原则可借鉴但不直接复用 |
| 章节一致性校验的 JSON 格式 | 待与 fili-web 现有 `ChapterFeedback` 数据结构整合 |
